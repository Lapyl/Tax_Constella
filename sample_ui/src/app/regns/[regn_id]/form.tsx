...
type IFormAprvSchema = z.infer<typeof FormAprvSchema>;

type ICastAprvFormProps = { regn: IRegn };

export const CastAprvForm = ({ regn }: ICastAprvFormProps) => {
  ...
  const {
    ...
    getValues,
    watch
  } = useForm<IFormAprvSchema>({
    mode: 'onTouched',
    resolver: zodResolver(FormAprvSchema),
    defaultValues: { regnId: regn.id }
  });

  const registerField = buildRegisterField(register, formState, control);

  const onFormSubmit = handleSubmit(async (values) => {
    const response = await useToastAction(castAprv, {
      progress: (t) => t.info('Submitting aprv'),
      error: (t, e) => t.error(`Unknown error while submitting aprv: ${e}`)
    })(values);
    ...
    if (response?.errors.serverErrors) {
      if (
        ...
        toast.error(
          'Unable to cast the aprv, you need to hold balance in your wallet, check the project README.md for more info',
          { autoClose: false }
        );
        return;
      }
  ...
  const buildAndSignPayload = async () => {
    const validatedValues = FormAprvSchema.safeParse(getValues());
  ...
  return (
    <form className={styles.content} onSubmit={onFormSubmit}>
      <div className={styles.cards}>
        <Card variants={['full-width', 'padding-m']} header={'Tax Registration Data'}>
          <Input
            label="Name"
            description="Maximum of 128 characters"
            placeholder="Tax registration name"
            readOnly
            {...registerField('regnId')}
          />
          <Input
            label="Owner"
          ...
          <Input
            ...
            options={Object.keys(regn.result).map((option) => ({
              value: option
            }))}
