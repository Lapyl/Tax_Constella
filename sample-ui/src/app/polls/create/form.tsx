...
export const CreatePollForm = () => {
  ...
  const onFormSubmit = handleSubmit(async (values) => {
    const response = await useToastAction(createPoll, {
      progress: (t) => t.info('Creating tax registration form'),
      error: (t, e) => t.error(`Unknown error while submitting tax registration form: ${e}`)
    })(values);

    if (response?.errors.serverErrors) {
      if (response.errors.serverErrors.content.includes('InvalidEndSnapshot')) {
        toast.error(
          'Unable to create the tax registration form. Check the end snapshot ordinal, must be greater than the current snapshot',
          { autoClose: false }
        );
        return;
      }
      ...
      <div className={styles.cards}>
        <Card variants={['full-width', 'padding-m']} header={'Tax registration form'}>
          ...
          <Input
            ...
            description="When the tax registration form should start"
            placeholder="Enter an ordinal number"
            {...registerField('startSnapshotOrdinal')}
          />
          <Input
            label="End snaphot ordinal"
            description="When the tax registration form should end"
            placeholder="Enter an ordinal number"
            {...registerField('endSnapshotOrdinal')}
          />
          <Input
            label="Signature"
            description="Your account signature for this tax registration form"
            readOnly
